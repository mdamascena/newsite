...
function InfoItem({ rotulo, valor }) {
  return (
    <div className="rounded-lg border border-slate-200 p-3 dark:border-slate-800">
      ...
      <p className="mt-1 break-words text-sm font-semibold text-slate-900 dark:text-white">{valor}</p>
    </div>
  );
}
    ...
    <div className="space-y-6">
      <header className="flex items-start gap-3">
        ...
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-slate-950 dark:text-white sm:text-3xl">Sua conta</h1>
      ...
      <div className="grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
        <aside className="space-y-5">
          <article className="rounded-lg border border-slate-200 bg-white p-5 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <h2 className="mt-4 text-xl font-bold text-slate-950 dark:text-white">{cliente.nomeCompleto}</h2>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{cliente.cpf}</p>

            <div className="mt-6 rounded-lg bg-slate-50 p-4 text-left dark:bg-slate-950">
              <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-semibold text-slate-900 dark:text-white">Perfil completo</p>
                <span className="text-sm font-bold text-blue-700 dark:text-blue-300">{cliente.completude}%</span>
              </div>
              <div className="mt-3 h-2 rounded-full bg-slate-200 dark:bg-slate-800">
                <div className="h-2 rounded-full bg-blue-600 dark:bg-blue-400" style={{ width: `${cliente.completude}%` }} />
              </div>
              <p className="mt-3 text-xs leading-relaxed text-slate-500 dark:text-slate-400">Dados completos diminuem retrabalho na validacao da proposta.</p>
            </div>
          </article>
